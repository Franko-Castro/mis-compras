...
try {
    // Preparar la consulta para obtener los datos del usuario (incluyendo imagen de perfil, portada, ubicación y whatsapp)
    $stmt = $conn->prepare("SELECT id_usuario, nombre, email, foto_perfil, foto_portada, ubicacion, whatsapp, verificado, DATE_FORMAT(fecha_registro, '%d/%m/%Y') as fecha_registro FROM usuarios WHERE id_usuario = ?");
