...
if ($res) {
    while ($cat = $res->fetch_assoc()) {
        ...
        $stmt = $conn->prepare("SELECT p.id_producto, p.nombre, p.descripcion, p.precio, p.imagen, p.id_vendedor, u.nombre AS vendedor, u.verificado, u.whatsapp, DATE_FORMAT(p.fecha_publicacion, '%d/%m/%Y') AS fecha_publicacion
            FROM productos p
